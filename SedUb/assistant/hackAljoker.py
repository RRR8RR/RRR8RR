...
'''
mm = '''
قم بلأنضمام الى قناة رنثون @xLxLxLrr3
'''
...
keyboard = [
  ...
  [
    Button.url("سورس رنثون ", "https://t.me/xLxLxLrr3")
    ]
]
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/BxxBxxL")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        file.write(i + "\n\nDetails BY @xLxLxLrr3")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\nشكراً لأستخدامك سورس رنثون ❤️. \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس رنثون", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك رنثون.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
      	await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك رنثون.", buttons=keyboard)
      else:
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/xLxLxLrr3")
        ]
    ]
