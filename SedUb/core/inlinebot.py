...
def main_menu():
    text = f"**- [سـورس رنثون](https://t.me/xLxLxLrr3)\
        \n المسـاعد\
        \n\
        \n◽ رنثون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("l313l"):
            buttons = [
                (
                    ...
                    Button.url(" SedUb UsᴇʀBoᴛ", "https://t.me/xLxLxLrr3"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس رنثون 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس رنثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس رنثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© SedUb Help",
                description="Help menu for SedUb",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/xLxLxLrr3"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/xLxLxLrr7",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت رنثون 🧸♥](https://t.me/xLxLxLrr3)**\
            \n\
            \n❤ رنثون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/xLxLxLrr7)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
