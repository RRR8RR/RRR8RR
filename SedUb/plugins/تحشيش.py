...
#كـتابة المـلف وتعديل.    :   السيد علش.   اخمط وسمي روحك مطور فرخي 😂
...
async def permalink(mention):
    ...
    if not user:
        return
    SedUb = user.last_name.replace("\u2060", "") if user.last_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"🚻 ** ᯽︙  المستخدم => • ** [{SedUb}](tg://user?id={user.id}) \n ☑️ **᯽︙  تم رفعها مرتك بواسطه  :**{my_mention} 👰🏼‍♀️.\n**᯽︙  يلا حبيبي امشي نخلف بيبي 👶🏻🤤** ")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 6186856384:
        return await edit_or_reply(mention, f"**- لكك دي هذا المطور**")
    SedUb = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
async def permalink(mention):
    await edit_or_reply(mention, f"سماحة السيد علش مطور سورس رنثون @xLxLxLrr3")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ الحلو** 「[{SedUb}](tg://user?id={user.id})」 \n**᯽︙  تـم رفعه مميز بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"᯽︙ ** لقد تم زواجك/ج من : **[{SedUb}](tg://user?id={user.id}) 💍\n**᯽︙  الف الف مبروك الان يمكنك اخذ راحتك ** ")
...
BxxBxxL = [6186856384, 6186856384, 6186856384, 6186856384]
@l313l.on(events.NewMessage(incoming=True))
async def Hussein(event):
    if event.reply_to and event.sender_id in BxxBxxL:
       ...
       if owner_id == l313l.uid:
           if event.message.message == "منصب؟":
               ...
               await event.reply("** السيد علش @BxxBxxL تاج راسي ❤️**")
...
async def permalink(mention):
    ...
    me = await mention.client.get_me()
    my_first = me.first_name
    BxxBxxL = random.choice(rehu)
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙الهمسة من المستخدم [{SedUb}](tg://user?id={user.id}) تم كشفها بنجاح ✓**\n**᯽︙  الهمسة هي : {BxxBxxL} ** ")
