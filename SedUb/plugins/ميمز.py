#all write Codes By Team Aljoker @xLxLxLrr3
#By Hussein @BxxBxxL
import asyncio
import random
import base64
...
async def aljoker313(joker313):
  ...
  await joker313.client.send_file(joker313.chat_id,url,caption="᯽︙ BY : @xLxLxLrr3 🎀",parse_mode="html")
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @xLxLxLrr3 🎀",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="قران$"))
async def jepvois(vois):
  rl = random.randint(2,101)
  url = f"https://t.me/QuraanJep/{rl}"
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @xLxLxLrr3 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption="᯽︙ THEME BY : @xLxLxLrr3 🎊",parse_mode="html")
  await theme.delete()
...
async def Hussein(event):
    ...
    BxxBxxL = event.pattern_match.group(2)
    aljoker_links[BxxBxxL] = url
    await event.edit(f"**᯽︙ تم اضافة البصمة {BxxBxxL} بنجاح ✓ **")
...
async def Hussein(event):
    BxxBxxL = event.pattern_match.group(1)
    Joker = await reply_id(event)
    if BxxBxxL in aljoker_links:
        url = aljoker_links[BxxBxxL]
...
async def delete_aljoker(event):
    BxxBxxL = event.pattern_match.group(1)
    if BxxBxxL in aljoker_links:
        del aljoker_links[BxxBxxL]
        with open('aljoker_links.json', 'w') as file:
            json.dump(aljoker_links, file)
        await event.edit(f"**᯽︙ تم حذف البصمة '{BxxBxxL}' بنجاح ✓**")
    else:
        await event.edit(f"**᯽︙ هذه البصمة '{BxxBxxL}' غير موجودة في القائمة**")
    joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
    joker = Get(joker)
    try:
        await event.client(joker)
...
async def list_aljoker(event):
    if aljoker_links:
        ...
        for BxxBxxL, url in aljoker_links.items():
            message += f"- البصمة : .`{BxxBxxL}`\n"
    else:
